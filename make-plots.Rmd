...
```
# Descriptive analysis of SNVs - Figure 2 

```{r figure-2, echo=FALSE, warning=FALSE, message=FALSE, eval=T}
...
ggsave(file.path(params$plot_path, "Figure_2.pdf"), 
       p_comb,  
       width = 210,
       height = 210,
       units = "mm",
...
ggsave(file.path(params$plot_path, "Supplementary-Figure-1.pdf"), 
       p,  
       width = 210,
       height = 210,
       units = "mm",
...
pdf(file.path(params$plot_path, "Supplementary-Figure-2.pdf"))
grid.newpage()
...
write.xlsx(concordance_df_expanded %>% as.data.frame(), 
           file.path(params$plot_path, "plot-source-files",
                     "Supplementary-Figure-3-data.xlsx"), 
...
p = Heatmap(concordance_matr, show_row_dend = F, na_col="#cfcfcf", show_row_names = F, 
        ...
        heatmap_legend_param = list(by_row=T, color_bar="discrete", at = c(NA, -1,1),
                                    labels = c( "N/A", "Discordant", "Concordant"),
                                    title_gp = grid::gpar(fontface = "plain", fontsize = 9) 
                                    ),
        ) 


pdf(file.path(params$plot_path, "Supplementary-Figure-3.pdf")) 
draw(p)
dev.off()
```
...
write.xlsx(concordance_df_tricho %>% as.data.frame(), 
           file.path(params$plot_path, "plot-source-files",
                     "Supplementary-Figure-4-data.xlsx"), 
           sheetName = str_c("Source data"),
           row.names = F, append = F)
...
pdf(file.path(params$plot_path, "Supplementary-Figure-4.pdf")) 
draw(p)
dev.off()
```
...
ggsave(v, filename = file.path(params$plot_path, "Supplementary-Figure-5.pdf"))
```
