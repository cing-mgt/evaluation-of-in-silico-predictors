...
compute_metrics_for_discrete = function (tool,
   ...
   results = tibble(
      ...
      YR = YR,
      Accuracy = ACC,
      Sensitivity = rval$detail$se$est,
      Specificity = rval$detail$sp$est,
      PPV = rval$detail$pv.pos$est,
      NPV = rval$detail$pv.neg$est,
      OPP = OPP,
      MCC = MCC,
      `LR+` = rval$detail$lr.pos$est,
      `LR+_CI_lower` = rval$detail$lr.pos$lower,
      `LR+_CI_upper` = rval$detail$lr.pos$upper,
      `LR-` = rval$detail$lr.neg$est,
      `LR-_CI_lower` = rval$detail$lr.neg$lower,
      `LR-_CI_upper` = rval$detail$lr.neg$upper
   )
